	...
	$.extend(Valid.prototype, {
		...
		validate: function() {
			...
			$.each(this.errors, function(field_name, error) {
				...
				// muzongyan modify start
        if($input.parent('.input-group').length) {
          $input.parent().siblings('span.help-block').html('');
        } else {
          $input.siblings('span.help-block').html('');
        }
        // muzongyan modify end
				...
				if (self.options.wrapper !== null) {
				  // muzongyan comment
					...
					// muzongyan modify start
	        if($input.parent('.input-group').length) {
	          $input.parent().closest(self.options.wrapper).removeClass('has-error');
	        } else {
	          $input.closest(self.options.wrapper).removeClass('has-error');
	        }
	        // muzongyan modify end
		...
		show: function(errors) {
			...
			$.each(errors, function(k, v) {
				...
				if (self.options.wrapper !== null) {
				  ...
				  // muzongyan modify start
				  if($input.parent('.input-group').length) {
				    $input.parent().closest(self.options.wrapper).addClass('has-error');
          } else {
            $input.closest(self.options.wrapper).addClass('has-error');
          }
				  // muzongyan modify end
